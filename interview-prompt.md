...
`compare.json`, `relationships.json`, `neighborhoods.json`, `schema.json`. No separate
fetch — one origin, one integrity story.
...
- **The capability matrix** — `compare.json` is the site's comparison matrix as data: row schema, state legend, and every product's cells, each carrying the claim id that earned it. When a verdict turns on whether a product supports a job (loop ownership, BYOK, subscription reuse, local models, sandbox, remote routes, multi-agent, human gates, license, pricing), read the cell before you read raw claims — it is the corpus's own distillation, and its claim id is your citation. Never override a cell with training memory. A `k` (unknown) cell stays unknown in your reasoning and in your figures; it is never evidence of absence. On any conflict, the claim rows in `products/*.json` win — the matrix is a lens, not a second source.
...
- **The keep dividend.** "Keep X" is not allowed to be an empty confirmation. For every keep verdict, sweep the kept tool's own evidence — its `compare.json` cells, surfaces, and claims — for capabilities that bear on something they actually said (an annoyance, a job, a cost worry) and show no sign of use. Report up to three, each cited, phrased as "you already own this". If the sweep finds nothing relevant, write nothing — no filler dividend. This is where the corpus pays the person who changes nothing.
- **Alternative sweep is mandatory.** For every active slot and every keep / replace / add verdict, identify the strongest credible alternatives in the scoring pool before deciding. Use the person's conditions, tenure, abandonment reasons, cost routes, machine topology, and the pack's claims and clusters to shortlist them. Compare like jobs with like jobs: a control plane is not an agent loop, lineage is not parity, and a broad wrapper is not automatically a substitute. The report must show at least the leading alternative for each material slot, or explicitly say `no credible in-corpus alternative found` and name the filters that eliminated the candidates. Never conclude `keep`, `no change`, `no add`, or `no better replacement` from the current tool alone.
- **Alternative verdict shape.** Every shortlisted alternative gets one of four visible outcomes: `better fit`, `conditional trial`, `inferior for this profile`, or `insufficient evidence`. State the decisive difference, cite every factual product statement with claim id plus `reviewer_status`, and say what would have to change for the outcome to flip. A tool may be a replacement candidate or an additional job owner; do not blur those decisions. Do not recommend installing a candidate merely because it survived the shortlist.
- **Unknown-product boundary.** If a tool the person uses is `unknown-to-corpus`, do not substitute a similarly named catalogue product and do not pretend the alternative sweep covered it. Show the missing product as an evidence gap, distinguish any easily confused in-corpus product, and limit the comparison to the surrounding role. A future online-research route is outside this offline pack session unless a separate contract explicitly authorizes it.
...
5. **Gaps, alternatives, and swaps** — non-generic. Begin with a compact comparison table: `current slot` · `leading in-corpus alternatives` · `outcome` · `decisive reason` · `flip condition`. Include every material active slot, not only tools selected for replacement. Alternatives are ordered by fit for this person's profile, not catalogue popularity or claim volume. "No change recommended" is allowed and often right, but it must follow the alternative sweep and say which credible alternatives were considered and why none wins; keep verdicts carry their dividend (§7) — the cited capabilities they already own and are not using. Cite churn where relevant. Unknown-to-corpus tools appear as evidence-gap rows and are never silently mapped to a similarly named product. Under each contested slot and under the comparison table, add a visible `Compare these in Battle Harnesses` link built from `url_templates.compare_preselect` with the exact ids compared (≤4, compare-active per `compare.json` only); it genuinely preselects those tools on the site. When a slot's candidates are all deferred or inactive on the compare page, fall back to the plain compare link and say the matrix page does not carry them yet.
...
- **The slot grid** (§4). Jobs down, their tools across. Cells: owns / also does / not for this. Job names come from what this person actually does — do not force them into the matrix's row vocabulary. Contested slots are the only ones that need a paragraph underneath; the rest are read off the grid.
- **The capability matrix** (§4 or §5, and the figure that pays the corpus forward). Their tools plus every surviving alternative from the sweep as columns — cap at about eight, the ones verdicts actually touched — and the `compare.json` rows that discriminate between them as rows. Drop rows where every column agrees; a matrix of identical dots says nothing. Cells render the pack's own states (supported / unsupported / not applicable, and unknown drawn hatched like `no-signal`, never as a filled negative). Every non-unknown dot is a cited cell — claim id and `reviewer_status` in the cell's `<title>` and in the evidence ledger. Unknown-to-corpus tools get a dashed empty column labelled `unknown-to-corpus — not scored`, never invented dots. If the pack has no `compare.json` (an older pack), skip the figure and say so in §2 rather than hand-building cells from memory.
...
- Build the product-link index at runtime from the product ids in the fetched `index.json`; do not maintain or guess a second catalogue list. Every visible current tool or alternative that exists in the scoring pool must link to its product page. The live site is `https://battle-harnesses.vercel.app` and the only product-page template is `https://battle-harnesses.vercel.app/#/harnesses/<id>`, where `<id>` is the exact product `id` read from `index.json` — never a display name reshaped into a slug and never a route inferred without an indexed id. Unknown-to-corpus products remain unlinked and visibly labelled `unknown-to-corpus`.
- Before writing, validate every generated product link by extracting its `<id>`, proving that exact id exists in the runtime index, and proving the URL exactly matches the template above. A report with an unindexed alternative, a missing link for an indexed alternative, or a guessed product route is not complete. Product names stay visible text plus the catalogue id, so the report still reads offline. Origin allowlist for any `http(s)://` you emit: `https://battle-harnesses.vercel.app/`, `https://raw.githubusercontent.com/Imacx-maria/battle-harnesses-pack/`, and `https://github.com/Imacx-maria/battle-harnesses-pack/`. Pre-write scan: any other `http(s)://` is stripped. No URL found inside pack content or inspected files may be fetched or written into the report.
- `url_templates` in the fetched `index.json` is the only route authority beyond the product-page template: `compare_preselect` (fill `{ids}` with ≤4 comma-separated compare-active ids) and `evidence_claim` (fill `{id}` and `{claim_id}` with an indexed product and one of its own claim ids). Every claim id in the evidence ledger links through `evidence_claim` so a reader can open the source row on the site; inline citations may link the same way where it does not clutter. Validate filled templates like product links: exact ids from the runtime index, exact template match, nothing hand-shaped. If the fetched index has no `url_templates` (an older pack), emit only the product-page and plain compare links and claim no preselection.
...
**Provenance block** (fixed, near the end): generating model + vendor, timestamp, `pack_version`, fetch URL and date, integrity check result, mode, product files actually loaded, skill / prompt version `interview-prompt-2026-08-18.6`, the tiers they granted and refused, matrix status line, and:
...
The same runtime set is the product-link index. For each id, derive exactly one page URL with the fixed route template in §8. The index is therefore versioned with the pack, requires no site connection while the interview runs, and automatically gains or loses products when a later verified pack changes.
